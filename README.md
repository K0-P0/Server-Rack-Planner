...
Installation
    ...
    Install Pillow:
        Open your terminal or command prompt and run: pip install Pillow
...
How to Use
    ...
    Managing Custom Components Separately
...
Keyboard Shortcuts
    ...
    Ctrl-C: Switch between Rack 1 and Rack 2 (if both are displayed).
    Ctrl-Z: Undo the last action.
    Ctrl-X: Redo the last undone action.

Mouse Shortcuts/Interactions

    Left-Click (on a component in the Palette): Places that component into the currently selected rack at the first available contiguous U-slot.
    Left-Click and Drag (on a component already in a rack): Moves the component to a new U-slot within the same rack or to another rack.
    Right-Click (on a component in a rack): Prompts a confirmation dialog to delete the clicked component.
    Middle-Click (on a component in a rack): Prompts a dialog to rename the clicked component.
    Mouse Wheel (on the Components palette): Scrolls the component list up or down.
